...
# Упростим функцию check_collision
def check_collision(pos):
    print(f"Checking collision for position: {pos}")  # отладочное сообщение
...
def insert_score(name, score, level):
    try:
        conn = psycopg2.connect(dbname='lab10', user='postgres', password='12345', host='localhost', port='5433')
        cur = conn.cursor()
        ...
        cur.close()
        conn.close()
        print(f"Score {score} inserted for player {name}.")
    except Exception as e:
        print(f"Error inserting score: {e}")

def get_scores(name):
    try:
        print(f"Fetching scores for player {name}...")
        conn = psycopg2.connect(dbname='lab10', user='postgres', password='12345', host='localhost', port='5433')
        cur = conn.cursor()
        ...
        cur.close()
        conn.close()
        print(f"Scores fetched: {results}")
        return results
    except Exception as e:
